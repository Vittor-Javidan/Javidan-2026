...
class Atributos():
  def __init__(self, path_sismica, resolucao, deltaz, path_velocidade, linha_barravelha, linha_camboriu):
    self.path = path_sismica
    # self.X = np.load(self.path_sismica)
    self.X = segyio.tools.cube(path_sismica)[0, :, :]
    self.resolucao = resolucao

    self.extent = [1, self.X.shape[0] * self.resolucao, deltaz[0], deltaz[1]]
    self.velocidade = segyio.tools.cube(path_velocidade)[0, :, :]
    self.barravelha = linha_barravelha
    self.camboriu = linha_camboriu
    self.resample(deltaz)
    self.segmenta()
  ...
  def resample(self, deltaz):
    self.crop_vel(deltaz)
    zoom_factors = (
        self.X.shape[0] / self.velocidade.shape[0],
        self.X.shape[1] / self.velocidade.shape[1]
    )
    self.velocidade_rs = scipy.ndimage.zoom(self.velocidade, zoom_factors, order=3)

  def crop_vel(self, deltaz):
    extent = deltaz
    h = int(self.velocidade.shape[1] * extent[1] / extent[0])
    self.velocidade = self.velocidade[:, h:]

  def segmenta(self):
    extent = self.extent[2:]
    barravelha = self.barravelha[:, 1]
    barravelha = (barravelha - extent[-1]) / (extent[-2] - extent[-1])
    camboriu = self.camboriu[:, 1]
    camboriu = (camboriu - extent[-1]) / (extent[-2] - extent[-1])

    segmentado = np.zeros(shape = self.X.shape)
    x = segmentado.shape[0]
    h = segmentado.shape[1]
    subsample_barravelha = np.linspace(0, barravelha.shape[0] - 1, x, dtype = int)
    subsample_camboriu = np.linspace(0, camboriu.shape[0] - 1, x, dtype = int)
    for j in range(x):
        coluna = np.zeros(shape = segmentado[j, :].shape)
        b = int(barravelha[subsample_barravelha[j]] * h)
        c = int(camboriu[subsample_camboriu[j]] * h)
        coluna[:b] = 0
        coluna[b:c] = 1
        coluna[c:] = 0
        segmentado[j, :] = coluna
    self.segmentado = segmentado