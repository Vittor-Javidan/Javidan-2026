...
def crop_vel(obj, deltaz):
    extent = deltaz
    h = int(obj.velocidade.shape[1] * extent[1] / extent[0])
    obj.velocidade = obj.velocidade[:, h:]

def plot_Fernanda(obj, atributo, color = 'seismic', linhas = False, zoom = None, percentil = (1, 99)):
    ...
    if linhas:
        plt.plot(
            (obj.barravelha[:, 0] + 1) * obj.resolucao,
            obj.barravelha[:, 1],
            color = 'lime',
            linewidth = 7
        )
        plt.plot(
            (obj.camboriu[:, 0] + 1) * obj.resolucao,
            obj.camboriu[:, 1],
            color = 'blueviolet',
            linewidth = 7
        )
    # plt.show()
    # plt.savefig('../output/{}_rf_velocidade.png'.format(obj.path.split('/')[-1].split('.')[0]), dpi = 300, );

def plot_VJ(obj, atributo, color = 'seismic', linhas = False, zoom = None, percentil = (1, 99)):
    print(obj.resolucao)
    plt.figure(figsize = [24, 10])
    if zoom is not None:
        plt.ylim(zoom)
    ...
    if linhas:
        plt.plot(
            (obj.barravelha[:, 0] + 1) * obj.resolucao / 3,
            obj.barravelha[:, 1],
            color = 'lime',
            linewidth = 7
        )
        plt.plot(
            (obj.camboriu[:, 0] + 1) * obj.resolucao / 3,
            obj.camboriu[:, 1],
            color = 'blueviolet',
            linewidth = 7
        )
